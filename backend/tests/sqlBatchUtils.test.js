...
const { splitSqlBatches, stripSqlBlockComments, stripSqlLineComments, isMeaningfulBatch } = require('../utils/sqlBatchUtils');
...
`;
        ...
        expect(meaningful.map((b) => stripSqlBlockComments(stripSqlLineComments(b)).trim()))
    ...
    it('strips line comments without regex', () => {
        expect(stripSqlLineComments('SELECT 1 -- note')).toBe('SELECT 1 ');
        expect(stripSqlLineComments('-- only')).toBe('');
    });
});
